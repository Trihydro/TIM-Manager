...
public class MilepostRepositoryImplementation implements MilepostRepository {
    ...
    public Collection<Milepost> getPath(String commonName, Double startLat, Double startLong, Double endLat,
        ...
        query += " where startMp.Direction in ['D','B']";
        ...
        query += " where endMp.Direction in ['D', 'B']";
        query += " with startMp, endMp, distance(point({longitude:$endLong,latitude:$endLat}), point({longitude:endMp.Longitude,latitude:endMp.Latitude})) as d2 ";
        query += " with startMp, endMp, d2 ORDER BY d2 ASC LIMIT 1";
        query += " with startMp, endMp ";
        query += " call algo.shortestPath.stream(startMp,endMp) yield nodeId";
