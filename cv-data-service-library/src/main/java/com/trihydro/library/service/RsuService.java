...
public class RsuService extends CvDataServiceLibrary {
	...
	public static ArrayList<WydotRsu> selectAll() {
		...
		try {
			...
			// select all RSUs from RSU table
   			rs = statement.executeQuery("select * from rsu inner join rsu_vw on rsu.deviceid = rsu_vw.deviceid order by milepost asc");
	...
	public static ArrayList<WydotRsu> selectRsusByRoute(String route){
		...
		try {
			...
			// select all RSUs from RSU table
   			rs = statement.executeQuery("select * from rsu inner join rsu_vw on rsu.deviceid = rsu_vw.deviceid where rsu_vw.route like '%" + route + "%' order by milepost asc");
	...
	public static List<WydotRsu> selectActiveRSUs(){
		...
		try {
			...
			// select all RSUs that are labeled as 'Existing' in the WYDOT view
   			rs = statement.executeQuery("select rsu.*, rsu_vw.latitude, rsu_vw.longitude, rsu_vw.ipv4_address from rsu inner join rsu_vw on rsu.deviceid = rsu_vw.deviceid where rsu_vw.status = 'Existing'");
