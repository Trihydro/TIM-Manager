...
while IFS=, read -r latitude longitude milepost ipv4_address serial_number iss_scms primary_route make model rsu_credential snmp_credential snmp_version firmware_version target_firmware_version; do
    ...
    numRecords=`PGPASSWORD=$db_password psql -d $DB_NAME -U $DB_CVMANAGER_USER -h $DB_HOST -p $DB_PORT -Atc "select COUNT(*) FROM cvmanager.rsus WHERE serial_number='$serial_number';"`
    ...
    PGPASSWORD=$db_password psql -d $DB_NAME -U $DB_CVMANAGER_USER -h $DB_HOST -p $DB_PORT -c "INSERT INTO cvmanager.rsu_organization(rsu_id, organization_id) VALUES ((SELECT rsu_id FROM cvmanager.rsus WHERE serial_number='$serial_number'), $wydot_organization_id);"
    if [[ $? -ne 0 ]]; then
