...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    sudo psql -h $DB_HOST -U $DB_CVMANAGER_USER -d $DB_NAME -c "CREATE SCHEMA IF NOT EXISTS $PRIMARY_SCHEMA_NAME;"
        if [ $? -ne 0 ]; then
        echo "Something went wrong while creating the schemas. Restore cancelled."
        exit 1
    fi
    sudo psql -U $DB_CVMANAGER_USER -d $DB_NAME -c "CREATE SCHEMA IF NOT EXISTS $KEYCLOAK_SCHEMA_NAME;"
    if [ $? -ne 0 ]; then
        echo "Something went wrong while creating the schemas. Restore cancelled."
        exit 1
    fi
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    sudo psql -h $DB_HOST -U $DB_CVMANAGER_USER -d $DB_NAME -f $CVMANAGER_SOURCE_DIR/$pathToTableScriptFromCvmanagerSourceDir
    if [ $? -ne 0 ]; then
...
echo "Enter postgres password for user $DB_CVMANAGER_USER to restore the primary schema:"
sudo docker run -it --rm -v $pathToPrimaryBackup:/pgdb-backup.dump --name temp-pgdb-restore-helper postgis/postgis:15-master pg_restore -n $PRIMARY_SCHEMA_NAME --clean --verbose -U $DB_CVMANAGER_USER -h $DB_HOST -p $DB_PORT -d $DB_NAME /pgdb-backup.dump
if [ $? -ne 0 ]; then
    sudo docker rm temp-pgdb-restore-helper 2> /dev/null
...
echo "Enter postgres password for user $DB_KEYCLOAK_USER to restore the keycloak schema:"
sudo docker run -it --rm -v $pathToKeycloakBackup:/pgdb-backup.dump --name temp-pgdb-restore-helper postgis/postgis:15-master pg_restore -n $KEYCLOAK_SCHEMA_NAME --clean --verbose -U $DB_KEYCLOAK_USER -h $DB_HOST -p $DB_PORT -d $DB_NAME /pgdb-backup.dump
if [ $? -ne 0 ]; then
    sudo docker rm temp-pgdb-restore-helper 2> /dev/null
    echo "The pg_restore command returned an error status code. Please check the state of the database to determine if the restore was successful."
fi
