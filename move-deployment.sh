...
# For example, cv-data-tasks-1.1.0-SNAPSHOT.jar would be deployed to cv-data-tasks/
#
# This script will:
# 1. Delete the previous backup (e.g. cv-data-tasks/cv-data-tasks-1.1.0-SNAPSHOT.jar.bak)
...
for path in ./deployment/*; do 
    file=$(basename $path)
    # Extract the directory name from the .jar (e.g. cv-data-tasks-1.1.0-SNAPSHOT.jar -> cv-data-tasks)
