...
public class TracManagerTest {
    ...
    public void TestSubmitDNMsgToTrac_SuccessFirstRound() throws IOException, URISyntaxException {
        // setup
        when(restTemplate.exchange(any(URI.class), Matchers.any(HttpMethod.class), Matchers.<HttpEntity<?>>any(),
                Matchers.<Class<String>>any())).thenReturn(new ResponseEntity<String>("ok", HttpStatus.OK));

        String value = new String(
                Files.readAllBytes(Paths.get(getClass().getResource("/distressNotification_OdeOutput.json").toURI())));
    ...
    public void TestSubmitDNMsgToTrac_SuccessSecondRound() throws IOException, URISyntaxException {
        // setup
        when(restTemplate.exchange(any(URI.class), Matchers.any(HttpMethod.class), Matchers.<HttpEntity<?>>any(),
                Matchers.<Class<String>>any())).thenThrow(new RestClientException("error"))
